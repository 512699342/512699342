...
func (mgr *SessionMgr) UpdateBrowserCookie(w http.ResponseWriter, sessionID string) {
	cookie := http.Cookie{Name: mgr.mCookieName, Value: sessionID, Path: "/", HttpOnly: true, MaxAge: int(mgr.mMaxLifeTime)}
	http.SetCookie(w, &cookie)
}
